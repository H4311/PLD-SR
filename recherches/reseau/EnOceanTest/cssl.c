		  ...
		  int stop)
{
    ...
    if ( (serial->fd = socket(AF_INET,SOCK_STREAM,0)) < 0)
    {
      printf("<Error> EnOcean - creation socket client\n");
      exit(0);
    }
    /* requete de connexion */
    if(connect( serial->fd,
                (struct sockaddr *)&serverSockAddr,
                sizeof(serverSockAddr)) < 0 )
    {
      printf("<Error> EnOcean - demande de connection\n");
      exit(0);
    }
    ...
    if(callback) {
		...
		/* F_SETSIG = 10 */
		fcntl(serial->fd,10,CSSL_SIGNAL);
